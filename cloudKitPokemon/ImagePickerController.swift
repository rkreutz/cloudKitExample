...
class ImagePickerController: UIImagePickerController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func imagePickerController(picker: UIImagePickerController, didFinishPickingImage image: UIImage, editingInfo: [String : AnyObject]?) {
        ...
        if image.size.width > 128.0 || image.size.height > 128.0 {
            var newSize: CGSize!
            if image.size.width > image.size.height {
                newSize = CGSizeMake(128.0 , image.size.height * 128.0/image.size.width)
            }
            else {
                newSize = CGSizeMake(image.size.width * 128.0/image.size.height, 128.0)
            }
